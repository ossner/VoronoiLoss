...
This chapter covers prior research on the topic of multi-instance semantic segmentation, covering ways in which the instance imbalance problem can be addressed. @sec_loss_taxonomy provides an overview of the current landscape of common loss functions in medical imaging and @sec_instance_losses shows how this taxonomy can be extended by instance-aware formulations. @sec_weight_maps shows how weight maps as additional components in loss calculation can be modeled to address instance-imbalance.
...
Shirokikh et al. @shirokikh2020universal introduced connected-component-based weight maps to address the instance-imbalance problem. Weight maps can be precomputed based on the labels and applied during loss calculation to change the contribution of individual voxels to the final loss value. In their proposed @iw approach, the weight map is calculated by equally distributing a fixed budget corresponding to the number of voxels in the image across all connected components as well as the background.
...
This work aims to position itself in the research gap created by these publications, providing a more thorough analysis of the use of Voronoi tessellation in loss calculation as well as an exploration of possible extensions to improve segmentation in multi-instance problems.