...
The stark differences in evaluation results in the 3D datasets point toward a fundamental difference the effect of weight maps have on numerically and morphologically diverse instances.

The 2D datasets exhibit a higher baseline segmentation performance, with weight maps only being able to marginally improve metrics and often worsening them. However, in @ag adaptive weighting based on the models current prediction showed general increases in @dsc ($0.813 arrow 0.825$), @rq ($0.746 arrow 0.768$), instance recall ($0.770 arrow 0.807$) and CCDice ($0.630 arrow 0.657$), but worsening @sqassd slightly ($0.386 arrow 0.401$).
