...
The presented results for @mets show that the region-wise inverse weighted map $W_"v_iw"$ increases instance recall drastically, while at the same time decreasing @rq. Since @rq is computed using both $"recall"_"inst"$ and $"precision"_"inst"$, it points toward a stark decrease in instance precision and a stark increase in $"FP"_"inst"$. @figsbmviwsample shows one test case as predicted by a model trained with $W_"v_iw"$. While the predictions are clustered more closely around the single label instance in the image, showing some learned behaviour, the extent of overprediction implies that the network failed to make relevant connections about this dataset and renders the model clinically unusable. 

Even though the weight map has similar effect on @wmh, increasing instance recall while reducing @rq, it is much less pronounced. This can be explained by the higher number of instances per sample in the dataset since they act as a regularization, distributing the large weight budget of 3D images across more Voronoi regions. This suggests that the relatively aggressive inverse weighting should only be applied when the expected number of instances per image exceeds a threshold sufficient to distribute the weight budget.
...
We further acknowledge the lack of an exhaustive search of several hyperparameters including the region-scaling penalty parameter $lambda$ in $W_"v_adaptive"$ across all datasets. $lambda=4$ was chosen based on limited experimentation as an exploration of the concept of adaptive weighting.
