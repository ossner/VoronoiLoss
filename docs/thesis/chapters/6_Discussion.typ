...
The presented results for @mets show that the region-wise inverse weighted map $W_"v_iw"$ increases instance recall drastically, while at the same time decreasing @rq, since this metric is computed using both $"recall"_"inst"$ and $"precision"_"inst"$, it points towards a stark decrease in instance precision and a stark increase in $"FP"_"inst"$. @figsbmviwsample shows one test case as predicted by a model trained with $W_"v_iw"$. While the predictions are clustered more closely around the label instance, showing some learned behaviour, the clear overprediction implies that the model failed to make relevant connections about this dataset and goes beyond any clinical usability. 
...
#figure(
  ...
  caption: [Slices of a prediction output of a model trained with $W_"v_iw"$ on a test image of the brain metastases dataset. True positive voxels in the prediction are marked in green #box(inset: 0pt, rect(width: 0.8em, height: 0.8em, fill: class_colors.at(0), stroke: 0.1pt)), @fp voxels in red #box(inset: 0pt, rect(width: 0.8em, height: 0.8em, fill: class_colors.at(2), stroke: 0.1pt)) and @fn voxels in blue #box(inset: 0pt, rect(width: 0.8em, height: 0.8em, fill: class_colors.at(1), stroke: 0.1pt)).
  ],
) <figsbmviwsample>
...
Another important point of discussion is the impact $W_"v_mountains"$ has on the segmentation performance of the 3D datsets, since it drastically improves most key metrics in @wmh, but it fails to reliably generalize to @mets, reducing the @rq severely, in part likely due to a difference in the instance distribution and morphology among the pathologies. 
...
We further acknowledge the lack of an exhaustive search of several hyperparameters including the region-scaling parameter $beta$ in $W_"v_adaptive"$ across all datasets. $beta=4$ was chosen based on limited experimentation as an exploration of the concept of adaptive weighting.