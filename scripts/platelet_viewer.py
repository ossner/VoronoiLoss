...
for t, slice in enumerate(semantic_mask):
    ...
    target_mask = (slice == TARGET_LABEL_ID)
...
print("Voronoi Stack shape:", voronoi_stack.shape)
print("Voronoi Stack dtype:", voronoi_stack.dtype)
...
viewer.add_labels(
    voronoi_stack,
    name=f'Voronoi Regions for {platelet_em_index_to_class[TARGET_LABEL_ID]}',
...
viewer.add_points(
    all_centers,
    name=f'Centroids of Label {TARGET_LABEL_ID} ({platelet_em_index_to_class[TARGET_LABEL_ID]})',
    size=10,
    face_color='red',
    opacity=0.8
)
